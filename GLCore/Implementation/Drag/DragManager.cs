...
namespace GLCore
{
    ...
    {
        public ISafetyList<ILine> mDragBound = new SafetyList<ILine>();

        public ISafetyList<ILine> mDragPointBound = new SafetyList<ILine>();
        ...
        private IRecode<IPair> mPositionRecode = FactoryMode.Factory.Recode<IPair>(FactoryMode.Factory.Pair(), FactoryMode.Factory.Pair());
        private EDragStatus mStatus = EDragStatus.Idle;

        /// <summary>
        /// 拖曳對象的邊緣
        /// </summary>
        public ISafetyList<ILine> DragBound { get { lock (mKey) return mDragBound; } }

        /// <summary>
        /// 當下拖曳點的邊緣
        /// </summary>
        public ISafetyList<ILine> DragPointBound { get { lock (mKey) return mDragPointBound; } }
            ...
            lock (mKey)
            {
                try
                {
                    if (Status == EDragStatus.Dragging && mDragTarget != null)
                    {
                        ...
                        mPositionRecode.NewData.X = newPosition.X;
                        mPositionRecode.NewData.Y = newPosition.Y;
                        IPair pos = mDragPoints[mDragPointsIndex].Point.Add(mPositionRecode.NewData).Subtraction(mPositionRecode.OldData);
        ...
        public void Draw(OpenGL gl)
        {
            lock (mKey)
            {
                ...
                gl.Begin(OpenGL.GL_LINES);
                {
                    mDragPointBound.SafetyForLoop((item) =>
                    {
                        gl.Vertex(item.Begin.X, item.Begin.Y, z);
                        gl.Vertex(item.End.X, item.End.Y, z);
                    });
            ...
            lock (mKey)
            {
                if (Status == EDragStatus.Ready && mDragTarget != null)
                {
                    ...
                    for (int ii = 0; ii < mDragPoints.Length; ++ii)
                    {
                        ...
                        {
                            mPositionRecode.NewData.X = position.X;
                            mPositionRecode.NewData.Y = position.Y;
                            mPositionRecode.Push();
        ...
        private void UpdateBound()
        {
            lock (mKey)
            {
                ...
                if (Status == EDragStatus.Dragging || Status == EDragStatus.Ready && mDragPoints != null)
                {
                    ...
                    {
                        IArea area = FactoryMode.Factory.Area(item.Point, item.Size.X, item.Size.Y);
